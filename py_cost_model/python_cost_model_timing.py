...
# Routines to help in timing the execution of various code fragments or routines, and to
...
def test_number():
    ...
    trials = 1000
    for D in param_list:
        #@orig svgr: t = timeit.Timer("string.atoi(x)", "import string;x='1'*%(n)s" % D)
    ...
    trials = 1000
    for D in param_list:
        #@orig svgr: t = timeit.Timer("x*x", "x=(2**%(n)s)/3" % D)
    ...
    trials = 1000
    for D in param_list:
        #@orig svgr: t = timeit.Timer("w/x", "w=(2**(2*%(n)s));x=(2**(%(n)s))" % D)
    ...
    #@orig svgr: spec_string = "1000<=n<=1000000"
    spec_string = "1000<=n<=10000"
    growth_factor = 2
    print("Spec_string: ", spec_string, "by factors of", growth_factor)
    var_list, param_list = make_param_list(spec_string, growth_factor)
...
def test_string():
    ...
    trials = 1000
    for D in param_list:
        #@orig. svgr, t = timeit.Timer("s[0:%(n)s/2]" % D, "s='0'*%(n)s" % D)
        t = timeit.Timer("s[0:%(n)s//2]" % D, "s='0'*%(n)s" % D)
        run_times.append(t.timeit(trials) * 1e6 / float(trials))
        #t.timeit(trials) #* 1e6 #/ float(trials)
        #xxx = t.timeit(trials)
        #run_times.append(xxx)
    fit(var_list, param_list, run_times, f_list)
    ...
    trials = 1000
    for D in param_list:
        #@orig svgr: t = timeit.Timer("string.translate(s,T)" % D,
        #                 "s='0'*%(n)s;import string;T=string.maketrans('1','2')" % D)
        t = timeit.Timer("str.translate(s,T)" % D,
                         "s='0'*%(n)s;import string;T=str.maketrans('1','2')" % D)
        run_times.append(t.timeit(trials) * 1e6 / float(trials))
    fit(var_list, param_list, run_times, f_list)
...
def test_list():
    ...
    trials = 2000
    for D in param_list:
        #@orig svgr: t = timeit.Timer("L[0:%(n)s/2]" % D, "L=[0]*%(n)s" % D)
    ...
    trials = 2000
    for D in param_list:
        #@orig svgr: t = timeit.Timer("L[0:%(n)s/2]=L[1:1+%(n)s/2]" % D, "L=[0]*%(n)s" % D)
